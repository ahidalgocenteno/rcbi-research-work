...
class VOC2007(Dataset):
    def __init__(self, root, phase, transform=None, inp_name=None):
        ...
        if self.inp_name:
            inp_name = os.path.join(self.inp_name, 'voc_glove_word2vec.pkl')
    ...
    def __getitem__(self, index):
        ...
        if self.inp_name:
            data = {'image':img, 'name': filename, 'target': target, 'inp':self.inp}
        else:
            data = {'image':img, 'name': filename, 'target': target}
        return data
